...
// 🔧 Use backend in production, mock data only in local dev if needed
...
export const api = {
  async getSongs() {
    ...
    const r = await fetch(`${BASE_URL}/songs`);
    return r.json();
  },

  async getPlaylists() {
    if (!USE_BACKEND) return mockPlaylists;
    const r = await fetch(`${BASE_URL}/playlists`);
    return r.json();
  },
  ...
  async getPlaylistById(id) {
    ...
    const r = await fetch(`${BASE_URL}/playlists/${id}`);
    return r.json();
  },
  ...
  async search(query) {
    ...
    const r = await fetch(`${BASE_URL}/search?q=${encodeURIComponent(query)}`);
    return r.json();
  },
};
